...
(() => {
  'use strict';
  const L=window.LESSON,D=window.COURSE_DATA,M=window.METRO,T=window.TAXI;
  ...
  const source=k=>L.sources[k], baseline=D.baselines(M.rows);
  ...
  function baselineLab(root){
    root.innerHTML=labHeader('预测误差会集中在哪些钟点？')+controls([select('hour','测试时段',[['all','全天'],...hourLabels.map((t,i)=>[String(i),t])],'all')])+'<div class="plot"></div><div class="stats"></div><div class="feedback"></div><details><summary>计算规则与可复现代码</summary><p>2017年训练，2018年1至9月测试。基线A使用训练样本全局均值。基线B使用训练样本中相同星期几、相同小时的均值。MAE是测试样本中预测值与观测值绝对差的平均。</p><a href="practice/reproduce.py" download>下载真实数据复现脚本</a></details>';
    function draw(){const h=$('select',root).value,rs=baseline.predictions.filter(r=>h==='all'||r.hour===+h),a=D.mean(rs.map(r=>Math.abs(r.volume-r.global))),b=D.mean(rs.map(r=>Math.abs(r.volume-r.periodic)));
      graph($('.plot',root),{labels:['全局均值','星期几×小时'],series:[{name:'测试集MAE',values:[a,b],color:'#087e82'}],bars:true,title:'两个历史均值基线的测试误差'});
      $('.stats',root).innerHTML=`<span>测试小时<strong>${rs.length}</strong></span><span>基线A<strong>${fmt(a,1)}</strong></span><span>基线B<strong>${fmt(b,1)}</strong></span>`;
      feedback(root,'训练参数始终只来自2017年，切换测试钟点不会重新训练。当前误差仅反映此数据、此划分与此基线，不能解释为方法通用排名。');
    }root.addEventListener('change',draw);draw();
  }
  function datasetLab(root){
    const rows=[['高速公路车辆轨迹','对象与道路环境不匹配。可以研究车辆运动，但无法直接回答校园行人夜间过街。'],['白天街景语义分割','能提供静态语义信息，但缺少夜间条件和连续行为信息。'],['经授权的夜间路口连续视频','与目标更接近。仍需行人标注、时间信息、视角范围与隐私合规安排。']];
  ...
  function article(s,i){return `<article class="lesson-article" id="slide-${i}" data-slide="${i}"><div class="case-label">${s.case?esc(s.case):'CHAPTER 01 / '+esc(s.section==='start'?'导览':s.section)}</div><h2>${esc(s.title)}</h2>${s.lead?`<p class="lead">${esc(s.lead)}</p>`:''}${s.numbers?`<div class="numbers">${s.numbers.map(([v,l])=>`<div><strong>${v}</strong><span>${l}</span></div>`).join('')}</div>`:''}${s.table?table(s.table):''}${s.body?`<div class="body-lines">${s.body.map((v,j)=>`<p class="body-line">${['rows','steps','closing'].includes(s.kind)?`<span class="index">0${j+1}</span>`:''}${esc(v)}</p>`).join('')}</div>`:''}${s.kind==='video'?videoHTML(s.video):''}${s.chart==='night'?'<div data-static-chart="night"></div>':''}${s.lab?`<div class="lab" data-lab="${s.lab}"></div>`:''}${s.link?`<p><a href="${source(s.link).url}" target="_blank" rel="noopener noreferrer">打开${esc(source(s.link).title)}</a></p>`:''}${s.takeaway?`<div class="takeaway">${esc(s.takeaway)}</div>`:''}${cite(s.sources)}<details class="teacher-note"><summary>教师提示与课堂追问</summary><p>${esc(s.notes||'')}</p></details></article>`;}
  function home(){return `<section class="hero"><p class="eyebrow">开放课堂 / 第1章</p><h1>交通数据挖掘<br>从一个问题开始</h1><p>为什么无事故也会堵车？<br>夜间交通又能告诉我们什么？</p><span class="caption">概念插画，非实际地点影像</span></section><div class="intro-line"><span><strong>${L.slides.length}</strong>页课堂讲义</span><span><strong>13</strong>个案例情境</span><span><strong>3</strong>个研究视频入口</span></div><p>以《交通数据挖掘理论与应用》第一章为主线。真实数据可筛选，概念可动手探索，每个案例都交代“能说明什么”和“还不能说明什么”。</p><div class="controls"><a class="download" href="#1.1">开始第一节</a><a href="chapter-01.pptx" download>下载可编辑PPT</a><a href="offline-course.zip" download>下载离线网页包</a></div>${article(L.slides[1],1)}<section class="lesson-article"><p class="eyebrow">案例索引</p><h2>先选一个你感兴趣的问题</h2><div class="case-list">${L.slides.map((s,i)=>s.case?`<a href="#s${i}"><span>${s.case.split(' ')[0]}</span>${esc(s.title)}</a>`:'').join('')}</div></section><p class="subtle">两组真实数据：UCI I-94小时交通量与NYC TLC绿出租车月度记录。教学模拟和练习数据均单独标注。核心交互可以离线使用，第三方视频需联网。按“课堂投影”可逐页展示，左右方向键翻页。</p>`;}
  ...
  function route(){const h=decodeURIComponent(location.hash.slice(1));let target=null;if(/^s\d+$/.test(h)){target=Math.min(L.slides.length-1,+h.slice(1));slideIndex=target;section=L.slides[target].section;}else if(L.sections.some(s=>s[0]===h)){section=h;slideIndex=L.slides.findIndex(s=>s.section===section);}else {section='start';slideIndex=0;}
