...
本轮章节算法另增20张实证表，五份`chapters/data/algorithms/ch4.json`至`ch8.json`保存来源SHA-256、实际依赖版本、完整逐条预测或标签。`chapters/python/case_algorithms.py`直接读取同一真实快照运行，`build-algorithm-evidence.mjs`生成网页摘要；无新增虚构交通观测。第5章人为遮盖是显式压力测试，不是补造未知事故；第8章匿名点重关联只用时间和坐标，源ID用于事后核查。完整参数、训练切分、许可与局限见[ALGORITHMS.md](ALGORITHMS.md)。原有30表作为前置分析保留，不将不同协议的分数拼接。
...
日画像基础聚类运行`projects/python/extensions.py --project taxi`，使用scikit-learn 1.9.1、random_state=42、n_init=10，对31日×168维日内占比比较K=2/3/4/5。`case-cluster-comparison.csv`保存轮廓系数，`case-day-clusters-4.csv`保存K=4日期分组，`case-cluster-centers-4.csv`保存中心。新版另以`case_algorithms.py --chapter 7`比较三个种子并计算ARI，完整结果见`ch7.json`。跨月份验证仍未实施；这些均为TLC真实快照的派生结果，许可沿用原始条款。

第三章主结果采用1000次重采样，区分旧500次示例。第六章原1小时基线表使用2190个目标，原多跨度及本轮ARIMA/SARIMA统一使用2169个共同目标，两种口径不混用。第八章增加匿名点重关联与源ID一致性核查，但仍无独立视频/人工真值，不填写完整视觉系统准确率。
