...
const references=(c,s)=>[['原始数据与官方说明',c.source],...(s.references||[])];
...
export function renderCaseStudy(c,p,target,note){
 ...
 +'<nav class="case-toc" aria-label="案例分析目录">'+CASE_SECTIONS.map(([id,title],i)=>'<button type="button" class="outline" data-case-section="'+id+'"><span>'+String(i+1).padStart(2,'0')+'</span>'+title+'</button>').join('')+'</nav>'+sections
...
export function caseToMarkdown(c){
 ...
 ...CASE_SECTIONS.flatMap(([key,title])=>['## '+title,'',...(key==='method'?s.method.flatMap(([h,t],i)=>['### '+(i+1)+'. '+h,'',t,'']):key==='evaluation'?['结果口径：'+e.protocol,'',...e.tables.map(mdTable),...s.evaluation.flatMap(t=>[t,''])]:s[key].flatMap(t=>[t,''])),'',...(key==='problem'?[mdTable(scopeTable(s)),mdTable(indicatorTable(s))]:[])]),
